...
To solve the objective function, we want to encode both *the objective* ($\max_{a, b} \mathbf{a}^T\Sigma_{XY}\mathbf{b}$) and *the constraint* ($a^T\Sigma_{XX}a = 1, b^T\Sigma_{YY}b = 1$), we turn to *Lagrangian multiplier*.
